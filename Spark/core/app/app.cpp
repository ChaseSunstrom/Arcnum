...
void Application::add_scene(const std::string &name,
                            const SceneConfig &config) {
  auto &_scene_manager = Engine::get<SceneManager>();
  _scene_manager.add_scene(name, std::make_unique<Scene>(config));
}

void Application::set_delta_time(u64 delta_time) { s_delta_time = delta_time; }
...
f64 Application::get_fixed_delta_time() { return s_fixed_delta_time; }

f64 Application::get_last_frame_time() { return s_last_frame_time; }

f64 Application::get_total_time() { return s_total_time; }

u64 Application::get_tick_speed() { return s_tick_speed; }

void Application::set_fixed_delta_time(f64 time) { s_fixed_delta_time = time; }

void Application::set_tick_speed(u64 speed) { s_tick_speed = speed; }
