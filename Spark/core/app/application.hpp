...
class Application {
  public:
	...
	using ApplicationQueryEventFunction = std::function<void(Application&, Query<T>&, const MultiEventPtr<EventTypes...>&)>;
	...
	template <IsSystem T>
	Application& AddSystem(T* t);

	template <IsSystem T>
	Application& AddSystem(std::unique_ptr<T> t);

	template <IsEvent EventType>
	Application& AddEventFunction(const std::function<void(Application&, const EventPtr<EventType>&)>& fn, const FunctionSettings settings = {});

	template <IsEvent... EventTypes>
	Application& AddEventFunction(const std::function<void(Application&, const MultiEventPtr<EventTypes...>&)>& fn, const FunctionSettings settings = {});
	...
	template <IsComponent T, IsEvent EventType>
	Application& AddQueryEventFunction(const std::function<void(Application&, Query<T>&, const EventPtr<EventType>&)>& fn, const FunctionSettings settings = {});
...
template <IsEvent EventType>
Application& Application::AddEventFunction(const std::function<void(Application&, const EventPtr<EventType>&)>& fn, const FunctionSettings settings) {
...
template <IsEvent... EventTypes>
Application& Application::AddEventFunction(const std::function<void(Application&, const MultiEventPtr<EventTypes...>&)>& fn, const FunctionSettings settings) {
...
template <IsComponent T, IsEvent EventType>
Application& Application::AddQueryEventFunction(const std::function<void(Application&, Query<T>&, const EventPtr<EventType>&)>& fn, const FunctionSettings settings) {
...
template <IsSystem T, typename... Args>
Application& Application::AddSystem(Args&&... args) {
	m_ecs->AddSystem(std::make_unique<T>(*m_event_handler, std::forward<Args>(args)...));
	return *this;
}

template <IsSystem T>
Application& Application::AddSystem(T* t) {
	m_ecs->AddSystem(std::unique_ptr<T>(t));
	return *this;
}

template <IsSystem T>
Application& Application::AddSystem(std::unique_ptr<T> t) {
