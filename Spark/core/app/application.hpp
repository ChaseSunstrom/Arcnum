...
class Application
{
  public:
	...
	template <IsComponent T> using ApplicationQueryEventFunction = std::function<void(Application &, Query<T> &, const std::shared_ptr<Event>)>;

	Application(GraphicsAPI gapi, std::unique_ptr<ThreadPool> tp = std::make_unique<ThreadPool>(std::thread::hardware_concurrency()))
		: m_thread_pool(std::move(tp)), m_event_handler(std::make_unique<EventHandler>(*m_thread_pool)), m_ecs(std::make_unique<Ecs>(*m_event_handler)), m_gapi(gapi)
	{
	}
...
template <IsComponent T> Application &Application::AddQueryEventFunction(i64 event_type, const ApplicationQueryEventFunction<T> &fn, const FunctionSettings settings)
{
	...
	m_event_handler->SubscribeToEvent(
		event_type,
		[this, fn, &query_event_wrapper_ref, settings](const std::shared_ptr<Event> event)
		{
