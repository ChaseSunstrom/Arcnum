...
public:
  ...
  Audio &create(const std::string &name, const std::filesystem::path &source,
                bool loops = false, bool pause_on_create = true) {
    std::unique_ptr<Audio> s = std::make_unique<Audio>(m_Engine->play2D(
        source.string().c_str(), loops, pause_on_create, true));
    m_sounds[name] = std::move(s);
    return *m_sounds[name];
  }

  Audio &create(const std::string &name, const std::filesystem::path &source,
                const math::vec3 &position, bool loops = false,
                bool pause_on_create = true) {
    internal::vec3df position_3d(position.x, position.y, position.z);

    std::unique_ptr<Audio> s = std::make_unique<Audio>(m_Engine->play3D(
        source.string().c_str(), position_3d, loops, pause_on_create));

    m_sounds[name] = std::move(s);
    return *m_sounds[name];
  }

  Audio &get_sound(const std::string &name) { return *m_sounds[name]; }
  ...
  void play_sound(const std::string &name, const math::vec3 &position,
                  bool loops = false) {
    Audio &s = *m_sounds[name];
  ...
  void pause_sound(const std::string &name) { m_sounds[name]->pause(); }

  void resume_sound(const std::string &name) { m_sounds[name]->resume(); }

  void loop_sound(const std::string &name) { m_sounds[name]->loop(); }

  void stop_sound(const std::string &name) { m_sounds[name]->stop(); }

  void stop_all() { m_Engine->stopAllSounds(); }
  ...
  internal::ISoundEngine *get_Engine() const { return m_Engine; }

private:
  AudioManager() = default;

  ~AudioManager() { m_Engine->drop(); }
...
struct AudioComponent {
  ...
  AudioComponent(const std::string &name,
                 const std::function<bool()> &play_condition)
