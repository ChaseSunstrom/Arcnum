...
struct IComponentInfo {
  IComponentInfo(const std::type_index &type) : m_type(type) {}

  virtual ~IComponentInfo() = default;

  virtual const std::type_index &get_type() const { return m_type; }

  std::type_index m_type;
};

// For type information when returning all components in an entity
template <typename T> class ComponentInfo : public IComponentInfo {
public:
  ComponentInfo(const T &component)
      : IComponentInfo(typeid(T)), m_component(component) {}

  const std::type_index &get_type() const override { return m_type; }

  const T &get_component() const { return m_component; }
...
public:
  ...
  std::vector<std::optional<T>> &get_array() { return m_component_array; }
  ...
  void entity_destroyed(Entity entity) override {
    if (entity < m_component_array.size()) {
      m_component_array[entity] =
          T(); // Resets the component to its default state.
    }
  }
  ...
  bool has_component(Entity entity) const override {
    return entity < m_component_array.size() &&
           m_component_array[entity].has_value();
  }
  ...
  u32 size() const { return m_component_array.size(); }
...
public:
  ...
  template <typename T> constexpr void register_component() {
    std::string type = std::type_index(typeid(T)).name();
    m_components[type] = std::make_unique<ComponentArray<T>>();
  }

  template <typename... components> constexpr void register_components() {
  ...
  template <typename T> void set_component(Entity entity, const T &component) {
  ...
  template <typename T> bool has_component(Entity id) const {
    std::string type = std::type_index(typeid(T)).name();
    auto found = m_components.find(type);

    if (found != m_components.end()) {
      auto array = static_cast<ComponentArray<T> *>(found->second.get());
      return id < array->get_array().size() &&
             array->get_array()[id].has_value(); // Adjusted for optional
    }
    return false;
  }
