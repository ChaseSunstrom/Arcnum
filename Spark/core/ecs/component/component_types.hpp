...
namespace Spark {
enum class TextureType { TWO_D = 2, THREE_D = 3 };

struct Transform {
  Transform(const math::vec3 &position = math::vec3(0.0f),
    ...
    mat = math::rotate(mat, math::radians(rotation.z),
                       math::vec3(0.0f, 0.0f, 1.0f));
    mat = math::rotate(mat, math::radians(rotation.y),
                       math::vec3(0.0f, 1.0f, 0.0f));
    mat = math::rotate(mat, math::radians(rotation.x),
  ...
  void update_matrix(const math::vec3 &position, const math::vec3 &rotation,
                     const math::vec3 &scale) {
    m_transform = math::mat4(1.0f); // Reset to identity matrix
    m_transform = math::translate(m_transform, position);
    m_transform = math::rotate(m_transform, math::radians(rotation.z),
                               math::vec3(0.0f, 0.0f, 1.0f));
    m_transform = math::rotate(m_transform, math::radians(rotation.y),
                               math::vec3(0.0f, 1.0f, 0.0f));
    m_transform = math::rotate(m_transform, math::radians(rotation.x),
                               math::vec3(1.0f, 0.0f, 0.0f));
...
struct Material {
  Material(const math::vec4 &color, Texture &tex, i32 diffuse, i32 specular,
           i32 ambient, f32 shininess)
      : m_color(color), m_diffuse(diffuse), m_specular(specular),
        m_ambient(ambient), m_shininess(shininess), m_texture(tex) {}
...
public:
  ...
  Texture &create(const std::string &name, const std::string &path,
                  TextureType type = TextureType::TWO_D, bool flip = true);
...
public:
  ...
  Material &
  create(const std::string &name, const math::vec4 &color = math::vec4(1),
         const std::string &texture_name = "",
         const std::pair<std::optional<std::string>, std::optional<std::string>>
             &shader_paths = {std::nullopt, std::nullopt},
         i32 diffuse = 0, i32 specular = 0, i32 ambient = 1,
         f32 shininess = 0) {
    ...
    if (!texture_name.empty() &&
        m_texture_manager.get_textures().contains(texture_name)) {
  ...
  void destroy_material(const std::string &name) { m_materials.erase(name); }

private:
  MaterialManager() { create("__default__"); }
...
public:
  ...
  Mesh &create(const std::string &name, const std::vector<Vertex> &vertices,
               const std::vector<u32> indices = {}, Args &&...args) {
    if (m_meshes.contains(name)) {
      return *m_meshes[name];
    }

    if (get_current_window_type() == WindowType::VULKAN) {
      m_meshes[name] = std::make_unique<VulkanMesh>(
          vertices, indices, std::forward<Args>(args)...);
      return *m_meshes[name];
    }
  }
  ...
  void destroy_mesh(const std::string &name) { m_meshes.erase(name); }
...
void set_uniform(const std::string &uniform_name, const Material &material,
                 u32 shader_program);
