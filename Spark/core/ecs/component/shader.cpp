...
namespace Spark {
ShaderType ShaderWrapper::get_shader_type_from_extension(
...
VulkanShaderWrapper::VulkanShaderWrapper(ShaderType type,
                                         internal::VkShaderModule module) {
  m_pipeline_shader.sType =
      internal::VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
...
void ShaderWrapper::create_vulkan_shader(
    const std::filesystem::path &shader_code) {
  ...
  if (vkCreateShaderModule(vk_window.get_window_data().m_device, &create_info,
                           nullptr, &shader_module) != internal::VK_SUCCESS) {
    SPARK_ERROR("[SHADER] Failed to create shader module");
  }

  m_shader_variant =
      std::make_unique<VulkanShaderWrapper>(m_shader_type, shader_module);
}
