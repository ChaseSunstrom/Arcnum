    ...
    void Ecs::RemoveAllEntityComponents(const i64 id)
    {
        ...
        for (auto& [type, components] : m_components)
        {
            ...
            auto it = std::remove_if(components.begin(), components.end(),
                [id, &removed_components](const std::shared_ptr<Component>& component) {
            ...
            {
                m_event_handler.PublishEvent(EVENT_TYPE_COMPONENT_REMOVED,
                    std::make_shared<ComponentRemovedEvent>(entity, type, component));
            }
        }
    }
}