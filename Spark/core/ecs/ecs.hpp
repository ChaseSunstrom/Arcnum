...
namespace Spark {
	class Ecs {
	  public:
		...
		template<IsComponent T> void AddComponent(Entity& entity, const std::string& name, const T& component);
		...
		template<IsComponent T> std::vector<T>& GetComponents();

		template<IsComponent T> i64 GetComponentCount();

		template<IsComponent T> T& GetComponent(Entity& entity, const std::string& name);
	...
	template<IsComponent T> T& Ecs::GetComponent(Entity& entity, const std::string& name) {
	...
	template<IsComponent... Ts> Entity& Ecs::MakeEntity(const std::pair<const char*, Ts>&... components) {
		u32 id = 0;
		if (m_recycled_ids.size() > 0) {
			id = m_recycled_ids.top();
		} else if (m_entities.size() > 0) {
			id = m_entities.size() > 0 ? m_entities.back().GetId() + 1 : 0;
		}
	...
	template<IsComponent T> void Ecs::AddComponent(Entity& entity, const std::string& name, const T& component) {
		auto& component_array = GetComponentArray<T>();
