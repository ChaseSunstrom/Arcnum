...
namespace spark
{
			...
			public Event
	{
		EntityUpdatedEvent(
				Entity entity,
				std::unordered_map<std::type_index, std::shared_ptr<IComponentInfo>> components) :
	...
	public:
		...
		template <typename T>
		inline ComponentArray<T>& get_component_array()
		{
		...
		Entity create_entity(const Components&... components)
		{
			...
			std::shared_ptr<EntityCreatedEvent> event = std::make_shared<EntityCreatedEvent>(
					new_entity,
