...
struct EntityCreatedEvent : public Event {
  EntityCreatedEvent(
      Entity entity,
      std::unordered_map<std::type_index, std::shared_ptr<IComponentInfo>>
          &&components_map)
...
struct EntityUpdatedEvent : public Event {
  EntityUpdatedEvent(
      ...
      std::unordered_map<std::type_index, std::shared_ptr<IComponentInfo>>
          components)
      : Event(ENTITY_UPDATED_EVENT), m_entity(entity),
...
struct EntityDestroyedEvent : public Event {
  EntityDestroyedEvent(Entity entity)
...
public:
  ...
  inline void add_component(Entity entity, const T &component) {
    ...
    std::shared_ptr<EntityUpdatedEvent> event =
        std::make_shared<EntityUpdatedEvent>(
            entity,
            std::unordered_map<std::type_index,
                               std::shared_ptr<IComponentInfo>>{
  ...
  inline T &register_update_system(Args &&...args) {
    return m_system_manager.register_update_system<T>(
        std::forward<Args>(args)...);
  }

  template <typename T, typename... Args>
  inline T &register_start_system(Args &&...systems) {
    return m_system_manager.register_start_system<T>(
        std::forward<Args>(systems)...);
  }

  template <typename T, typename... Args>
  inline T &register_shutdown_system(Args &&...systems) {
    return m_system_manager.register_shutdown_system<T>(
        std::forward<Args>(systems)...);
  }

  inline void start_systems() { m_system_manager.start_systems(); }

  inline void update_systems(f64 delta_time) {
    m_system_manager.update_systems(delta_time);
  }

  inline void shutdown_systems() { m_system_manager.shutdown_systems(); }

  inline Entity create_entity() { return m_entity_manager.create_entity(); }

  inline bool entity_is_renderable(Entity entity) {
    return has_component<MeshComponent>(entity) &&
           has_component<TransformComponent>(entity) &&
  ...
  inline void destroy_entity(Entity entity) {
    ...
    std::shared_ptr<EntityDestroyedEvent> event =
        std::make_shared<EntityDestroyedEvent>(entity);
  ...
  Entity create_entity(const Components &...components) {
    ...
    auto add_component = [this, &components_map,
                          &new_entity](const auto &component) {
      using component_type = std::decay_t<decltype(component)>;
      m_component_manager.add_component<component_type>(new_entity, component);

      components_map[typeid(component_type)] =
          std::make_shared<ComponentInfo<component_type>>(component);
    };

    (add_component(components), ...);

    std::shared_ptr<EntityCreatedEvent> event =
        std::make_shared<EntityCreatedEvent>(new_entity,
...
private:
  ECS() {
    ...
    register_update_system<AudioSystem>(
        &get_component_array<AudioComponent>().get_array());
  }
...
private:
  ...
  SERIALIZE_MEMBERS(ECS, m_component_manager, m_entity_manager,
                    m_system_manager)
};
} // namespace spark
