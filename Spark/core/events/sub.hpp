...
{
    ISubscription(i32 topic) : m_topic(topic), m_subscription_id(subscription_count++)
    {
    }
...
inline std::vector<std::shared_ptr<ISubscription>> subscriptions = std::vector<std::shared_ptr<ISubscription>>();

template <typename T> struct Subscription : ISubscription
{
    ...
    {
        auto it =
            std::find_if(subscriptions.begin(), subscriptions.end(), [this](const std::shared_ptr<ISubscription> &sub) {
  ...
  private:
    ...
    Subscription(i32 topic, std::function<void(std::shared_ptr<T>)> cb) : ISubscription(topic), callback(cb)
    {
    }
};
