...
#define SERIALIZE_MEMBERS(class_name, ...)                                                                             \
    ...
    {                                                                                                                  \
        BOOST_PP_SEQ_FOR_EACH_I(BOOST_SERIALIZATION_NVP_EACH, ~, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))                \
    }                                                                                                                  \
    struct class_name##_##state                                                                                        \
    {                                                                                                                  \
        BOOST_PP_SEQ_FOR_EACH_I(DECLARE_STATE_MEMBER, class_name, BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))               \
        friend class boost::serialization::access;                                                                     \
...
{
  protected:
    Serializeable(const std::string &type, uint16_t version) : m_type(type), m_version(version)
    {
    }
...
template <typename T> void save_to_file(const T &obj, const std::string &filepath)
{
...
{
  public:
    static void register_factory(const std::string &type, uint16_t version, std::unique_ptr<IPacketFactory> factory)
    {
    ...
    static std::unique_ptr<Serializeable> create_packet(const std::string &type, uint16_t version,
    ...
    static std::unordered_map<std::string, std::unique_ptr<IPacketFactory>> &get_factories()
    {
        static std::unordered_map<std::string, std::unique_ptr<IPacketFactory>> s_factories;
        return s_factories;
    }
...
#define REGISTER_PACKET_TYPE(NAMESPACE, PACKET_TYPE, VERSION)                                                          \
    ...
    static bool registered_##PACKET_TYPE = []() {                                                                      \
        ::Spark::net::PacketFactoryRegistry::register_factory(PacketTraits<NAMESPACE::PACKET_TYPE>::type(),            \
