...
#define DECLARE_STATE_MEMBER(r, data, i, elem) \
...
#define ASSIGN_FROM_CLASS(r, data, i, elem) \
    state.elem = this->elem;

#define ASSIGN_FROM_STATE(r, data, i, elem) \
...
{
	template <typename T>
	std::string serialize_to_binary(const T& obj)
	{
	...
	template <typename T>
	T deserialize_from_binary(const std::string& data)
	{
		T obj;
	...
	namespace net
	{
		...
		public:
			...
			static std::unique_ptr <Serializeable> create_packet(
					const std::string& type,
					uint16_t version,
...
#define REGISTER_PACKET_TYPE(NAMESPACE, PACKET_TYPE, VERSION) \
                ...
                template<> \
                struct ::PacketTraits<NAMESPACE::PACKET_TYPE> { \
