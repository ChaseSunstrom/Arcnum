...
class UDPClient {
public:
  UDPClient(const std::string &host = "127.0.0.1",
    ...
    m_socket.async_receive_from(
        ...
        [this](boost::system::error_code ec, std::size_t bytes_recvd) {
          if (!ec && bytes_recvd > 0) {
            ...
            SPARK_TRACE("[UDP CLIENT RECEIVED PACKET]: [TYPE]:"
            ...
            if (packet) {
              ...
              SPARK_FATAL("[UDP CLIENT UNABLE TO PROCESS PACKET]: [TYPE]:"
                          << type << " [VERSION]:" << std::to_string(version));
            }
...
class UDPServer {
public:
  UDPServer(const std::string &ip = "127.0.0.1",
            const std::string &port = "8080",
            asio::io_context &io_context = default_io_context())
      : m_socket(io_context), m_work(io_context), m_io_context(io_context) {
    udp::endpoint local_endpoint(asio::ip::make_address(ip),
  ...
  void broadcast(const Serializeable &packet) {
    ...
    for (const auto &client_endpoint : m_clients) {
      ...
      m_socket.async_send_to(
          asio::buffer(serialized_packet), client_endpoint,
...
private:
  ...
  asio::io_context::work
      m_work; // Ensure the io_context doesn't exit prematurely
