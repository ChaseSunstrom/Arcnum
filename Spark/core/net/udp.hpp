				...
				m_socket.async_receive_from(
						...
						[this](boost::system::error_code ec, std::size_t bytes_recvd)
						{
							if (!ec && bytes_recvd > 0)
							{
								...
								{
									SPARK_FATAL(
											"[UDP CLIENT UNABLE TO PROCESS PACKET]: [TYPE]:" << type << " [VERSION]:"
											                                                 << std::to_string(version));
								}
						...
						[this](boost::system::error_code ec, std::uint64_t bytes_recvd)
						{
							if (!ec && bytes_recvd > 0)
							{
								try
								{
									...
									{
										SPARK_FATAL("[UDP SERVER UNABLE TO PROCESS PACKET]: [TYPE]:" << type
										                                                             << " [VERSION]:"
										                                                             << std::to_string(
												                                                             version));
									}
			...
			void broadcast(const serializeable& packet)
			{
				...
				for (const auto& client_endpoint: m_clients)
				{
					...
					m_socket.async_send_to(
							...
							[serialized_packet = std::move(serialized_packet), client_endpoint](
									boost::system::error_code ec, u64 bytes_sent)
							{
								if (ec)
								{
									SPARK_ERROR(
											"[UDP SERVER BROADCAST]: Error sending packet to client: " << ec.message()
											                                                           << "[CLIENT]: "
											                                                           << client_endpoint);
								}
								else
								{
									SPARK_TRACE("[UDP SERVER BROADCAST]: Broadcasted packet: " << serialized_packet
									                                                           << "to [CLIENT]: "
									                                                           << client_endpoint);
								}
							});
