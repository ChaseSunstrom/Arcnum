                ...
                m_socket.async_receive_from(
                    ...
                    [this](boost::system::error_code ec, std::size_t bytes_recvd)
                    {
                        if (!ec && bytes_recvd > 0)
                        {
                            ...
                            SPARK_TRACE("[UDP CLIENT RECEIVED PACKET]: [TYPE]:" << type << " [VERSION]:" << std::to_string(version));
                            if (packet)
                            {
                                packet->process();
                                std::shared_ptr<udp_server_receive_event> event = std::make_shared<udp_server_receive_event>(std::move(packet));
                            ...
                            {
                                SPARK_FATAL("[UDP CLIENT UNABLE TO PROCESS PACKET]: [TYPE]:" << type << " [VERSION]:" << std::to_string(version));
                            }
