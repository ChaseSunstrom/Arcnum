...
namespace Spark
{
    ...
    template <typename T>
    static void GLUpdateMesh(std::vector<T>& old_data, const std::vector<T>& new_data, u32 bo)
    {
    ...
    {
        GLUpdateMesh<Vertex>(m_vertices, new_vertices, m_vbo);
    }

    void GLDynamicMesh::UpdateIndices(const std::vector<u32>& new_indices)
    {
        GLUpdateMesh<u32>(m_indices, new_indices, m_ebo);
    }
