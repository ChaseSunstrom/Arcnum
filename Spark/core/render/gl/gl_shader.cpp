...
namespace Spark {
GLVertexShader::GLVertexShader(const std::filesystem::path& path)
	: VertexShader(path) {
	Compile();
}

GLFragmentShader::GLFragmentShader(const std::filesystem::path& path)
	: FragmentShader(path) {
	Compile();
}

GLComputeShader::GLComputeShader(const std::filesystem::path& path)
	: ComputeShader(path) {
	Compile();
}
