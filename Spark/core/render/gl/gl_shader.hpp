...
namespace Spark
{
	...
	class GLVertexShader : public VertexShader, private GLShaderCommon
	{
	public:
	...
	class GLFragmentShader : public FragmentShader, private GLShaderCommon
	{
	public:
	...
	class GLComputeShader : public ComputeShader, private GLShaderCommon
	{
	public:
		friend class Manager<GLComputeShader>;
		GLComputeShader(const std::filesystem::path& path);
		~GLComputeShader();
		u32 GetId() const { return m_id; }
		void Compile() override;
		void Use() override { GLShaderCommon::Use(m_id); }
		...
		void SetF64(const std::string& name, const f64 value) override { GLShaderCommon::SetF64(m_id, name, value); }
		void SetBool(const std::string& name, const bool value) override { GLShaderCommon::SetBool(m_id, name, value); }
		void Dispatch(u32 num_groups_x, u32 num_groups_y, u32 num_groups_z) override { glDispatchCompute(num_groups_x, num_groups_y, num_groups_z); }
		void BindShaderStorageBlock(const std::string& block_name, u32 binding_point) override
		{
			u32 block_index = glGetProgramResourceIndex(m_id, GL_SHADER_STORAGE_BLOCK, block_name.c_str());
			if (block_index != GL_INVALID_INDEX) {
				glShaderStorageBlockBinding(m_id, block_index, binding_point);
			}
		}
	private:
		i32 m_id;
	...
	class GLRenderShader : public RenderShader, private GLShaderCommon
	{
	public:
		friend class Manager<GLRenderShader>;
		GLRenderShader(const std::vector<std::pair<ShaderStage, const std::filesystem::path>>& stage_paths) : RenderShader(stage_paths) {}
		~GLRenderShader();
		u32 GetId() const { return m_id; }
		void Compile() override;
		void Use() override { GLShaderCommon::Use(m_id); }
