...
namespace Spark {
	...
	template<> class Manager<StaticMesh> : public IManager {
	  public:
		...
		template<typename DerivedMesh, typename... Args> DerivedMesh& Create(const std::string& name, Args&&... args) {
			auto* mesh   = new DerivedMesh(std::forward<Args>(args)...);
			return static_cast<DerivedMesh&>(Register(name, std::unique_ptr<DerivedMesh>(mesh)));
		}
