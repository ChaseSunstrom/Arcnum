...
void Instancer::add_renderable(Entity e, const Scene &scene,
...
void Instancer::remove_renderable_for_entity(Entity e) {
  ...
  try {
    ...
    SPARK_ERROR(
        "[INSTANCER] Error removing renderable for entity: " << e.what());
  }
}
...
void Instancer::on_notify(std::shared_ptr<Event> event) {
  ...
  case ENTITY_CREATED_EVENT: {
    auto _entity_created_event =
        std::static_pointer_cast<EntityCreatedEvent>(event);
    ...
    if (ecs.has_component<TransformComponent>(e) &&
      ...
      auto material_name =
          ecs.get_component<MaterialComponent>(e).m_material_name;
  ...
  case ENTITY_UPDATED_EVENT: {
    auto _entity_updated_event =
        std::static_pointer_cast<EntityUpdatedEvent>(event);
  ...
  case ENTITY_DESTROYED_EVENT: {
    auto _entity_destroyed_event =
        std::static_pointer_cast<EntityDestroyedEvent>(event);
...
void Instancer::update_renderable(Entity e, Scene &scene) {
  ...
  if (m_entity_mesh_materials.find(e) != m_entity_mesh_materials.end()) {
    ...
    if (m_renderables.find(mesh_name) != m_renderables.end() &&
        m_renderables[mesh_name].find(material_name) !=
            m_renderables[mesh_name].end()) {
      // Update the transformation for the renderable object
      auto &transform_storage = m_renderables[mesh_name][material_name];
      if (transform_storage) {
        // Get the transform component from the ECS
        auto &transform =
            Engine::get<ECS>().get_component<TransformComponent>(e);
...
void Instancer::render_instanced(
    const std::vector<std::unique_ptr<Camera>> &cameras, Scene &scene) {
    ...
    for (auto &[material_name, transforms_ptr] : material_map) {
      ...
      if (get_current_window_type() == WindowType::VULKAN) {
        ...
        vkCmdBindVertexBuffers(
            vk_window.get_window_data()
                .m_command_buffers[vk_window.get_window_data().m_current_frame],
            0, 1, vertex_buffers, offsets);

        if (!_vulkan_mesh.m_indices.empty()) {
          vkCmdBindIndexBuffer(
              vk_window.get_window_data().m_command_buffers
                  [vk_window.get_window_data().m_current_frame],
          ...
          vkCmdBindDescriptorSets(
              ...
              vk_window.get_window_data().m_pipeline_layout, 0, 1,
              &vk_window.get_window_data().m_descriptor_sets
                   [vk_window.get_window_data().m_current_frame],
              0, nullptr);

          vkCmdDrawIndexed(vk_window.get_window_data().m_command_buffers
                               [vk_window.get_window_data().m_current_frame],
                           _vulkan_mesh.m_indices.size(),
                           transforms_ptr->m_data.size(), 0, 0, 0);
        }
          ...
          vkCmdDraw(vk_window.get_window_data().m_command_buffers
                        [vk_window.get_window_data().m_current_frame],
