...
namespace spark
{
	struct octree
	{
		octree(const math::vec3& center, f32 size, i32 depth = 0, octree* parent = nullptr) :
		...
		bool is_inside(const math::vec3& point, f32 size) const;
		...
		i32 determine_child(const math::vec3& point) const;
		i32 determine_child(const math::vec3& point, const math::vec3& center) const;
		void collect_visible(const frustum& frustum, std::vector<math::mat4>& visible) const;
		void grow(const math::vec3& position);
		math::vec3 calculate_child_center(i32 child_index) const;
		...
		const i32 m_max_components = 8;
		const i32 m_max_depth = 8;
	};
}
