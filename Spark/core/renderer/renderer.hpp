...
struct RendererSettings {
  RendererSettings(
      ...
      i32 min_width = 800, i32 min_height = 600, i32 max_width = 3840,
      i32 max_height = 2160, i32 texture_quality = 1, i32 shadow_quality = 1,
      ...
      bool occlusion_culling = true, i32 max_frames = 60, bool textures = true,
      bool anti_aliasing = true, bool shadows = true, bool reflections = true,
      bool hdr = true, bool bloom = true, bool volumetric_lighting = false,
      bool motion_blur = false, bool wireframe_mode = false,
      bool show_bounding_boxes = false, bool fps_counter = true,
      bool light_visualizer = false, i32 tessellation_quality = 0,
      i32 foliage_quality = 1, i32 water_quality = 1,
      i32 post_processing_quality = 1, i32 global_illumination_quality = 1,
      bool dynamic_resolution = false, i32 cpu_thread_usage = 1,
      bool gpu_async_compute = false, bool soft_particles = true,
      bool screen_space_reflections = true, bool depth_of_field = false,
      bool ambient_occlusion = true, bool color_grading = true,
      bool normals_visualizer = false, bool uv_debug_mode = false,
      bool physics_debug_drawer = false, bool lighting_only_mode = false,
      f32 ui_scale = 1.0f, bool subtitles = false, i32 text_size = 1)
      : m_resolution_width(resolution_width),
        m_resolution_height(resolution_height), m_min_width(min_width),
        m_min_height(min_height), m_max_width(max_width),
        m_max_height(max_height), m_texture_quality(texture_quality),
        m_shadow_quality(shadow_quality),
        m_reflection_quality(reflection_quality),
        m_anti_aliasing_level(anti_aliasing_level),
        m_lighting_quality(lighting_quality),
        m_volumetric_lighting_quality(volumetric_lighting_quality),
        ...
        m_textures(textures), m_anti_aliasing(anti_aliasing),
        m_shadows(shadows), m_reflections(reflections), m_hdr(hdr),
        m_bloom(bloom), m_volumetric_lighting(volumetric_lighting),
        m_motion_blur(motion_blur), m_wireframe_mode(wireframe_mode),
        m_show_bounding_boxes(show_bounding_boxes), m_fps_counter(fps_counter),
        m_light_visualizer(light_visualizer),
        m_tessellation_quality(tessellation_quality),
        m_foliage_quality(foliage_quality), m_water_quality(water_quality),
        m_post_processing_quality(post_processing_quality),
        m_global_illumination_quality(global_illumination_quality),
        m_dynamic_resolution(dynamic_resolution),
        m_soft_particles(soft_particles),
        m_screen_space_reflections(screen_space_reflections),
        m_depth_of_field(depth_of_field),
        m_ambient_occlusion(ambient_occlusion), m_color_grading(color_grading),
        m_normals_visualizer(normals_visualizer),
        m_uv_debug_mode(uv_debug_mode),
        m_physics_debug_drawer(physics_debug_drawer),
  ...
  bool m_dynamic_resolution =
      false; // Adjust resolution dynamically to maintain framerate

  // Additional Feature toggles
  bool m_soft_particles = true; // Toggle soft particles to prevent harsh
                                // intersections with geometry
  bool m_screen_space_reflections =
      true; // Toggle screen space reflections, can be performance-intensive
  bool m_depth_of_field = false; // Toggle depth of field effect
  bool m_ambient_occlusion =
      true; // Toggle ambient occlusion for more realistic shadows and depth
  bool m_color_grading = true; // Toggle color grading for cinematic looks

  // Extended Debugging and development
  bool m_normals_visualizer =
      false; // Visualize normal vectors, useful for debugging shading issues
  bool m_uv_debug_mode =
      false; // Display UV coordinates as colors, to debug texture mappings
  bool m_physics_debug_drawer =
      false; // Enable debug drawer for physics Engine (collision shapes, etc.)
  ...
  bool m_subtitles =
      false; // Enable subtitles for game dialogues or important sounds
  i32 m_text_size =
      1; // 0: Small, 1: Medium, 2: Large, for UI text size customization
};
...
private:
  std::unique_ptr<RendererSettings> m_settings =
      std::make_unique<RendererSettings>();
