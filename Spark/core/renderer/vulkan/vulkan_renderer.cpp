    ...
    for (auto &[mesh_name, material_map] : m_renderables)
    {
        for (auto &[material_name, transforms_ptr] : material_map)
        {
            ...
            if (is_current_api(API::VULKAN))
            {
                ...
                vkCmdBindVertexBuffers(window_data.m_command_buffers[window_data.m_current_frame], 0, 1,
                    vertex_buffers, offsets);

                if (!vulkan_mesh.m_indices.empty())
                {
                    vkCmdBindIndexBuffer(
                        window_data.m_command_buffers[window_data.m_current_frame],
                        vulkan_mesh.m_index_buffer, 0, Internal::VK_INDEX_TYPE_UINT32);

                    vkCmdBindDescriptorSets(
                        window_data.m_command_buffers[window_data.m_current_frame],
                        Internal::VK_PIPELINE_BIND_POINT_GRAPHICS, window_data.m_pipeline_layout, 0, 1,
                        &window_data.m_descriptor_sets[window_data.m_current_frame], 0,
                        nullptr);
