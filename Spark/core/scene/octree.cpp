...
namespace Spark
{
    Octree::Octree(glm::vec3 center, f32 width) : m_root(std::make_unique<Node>(center, width / 2)) {
        if (width > MAX_SIZE) {
    ...
    bool Octree::Remove(i64 entity_id) {
        ...
        if (it == m_entity_to_node.end()) return false;

        Node* node = it->second;
        auto& points = node->points;
        points.erase(std::remove_if(points.begin(), points.end(),
            [entity_id](const PointData& pd) { return pd.entity_id == entity_id; }),
            points.end());
    ...
    Octree::Node* Octree::Insert(Octree::Node* node, const PointData& point_data, i32 depth) {
        ...
        i32 index = 0;
        if (point_data.position.x > node->center.x) index |= 1;
        if (point_data.position.y > node->center.y) index |= 2;
        if (point_data.position.z > node->center.z) index |= 4;
    ...
    bool Octree::IsPointInBounds(const glm::vec3& point, const glm::vec3& center, f32 half_width) const {
        return glm::all(glm::lessThanEqual(glm::abs(point - center), glm::vec3(half_width)));
    }
    ...
    void Octree::QueryRange(Node* node, const glm::vec3& min, const glm::vec3& max, std::vector<PointData>& result) {
        ...
        if (!IntersectsRange(node->center, node->half_width, min, max)) return;
    ...
    bool Octree::IsPointInRange(const glm::vec3& point, const glm::vec3& min, const glm::vec3& max) {
