...
public:
  Octree(const math::vec3 &center, const math::vec3 &half_size,
         Octree *parent = nullptr)
      : Observer(true), SpatialPartition(), m_center(center),
        m_half_size(half_size), m_parent(parent) {
    if (!parent)
      register_observer();
  ...
  void add_entity(Entity e) override {
    ...
    math::vec3 position =
        extract_position(ecs.get_component<Transform>(e).m_transform);

    if (!point_is_inside(position)) {
      if (m_parent == nullptr) {              // Only the root can expand
    ...
    if (is_leaf()) {
      ...
      if (m_entities.size() > 8) { // Arbitrary threshold to subdivide
        subdivide();
      }
    } else {
  ...
  bool point_is_inside(const math::vec3 &point) const {
    return (point.x >= m_center.x - m_half_size.x &&
            point.x <= m_center.x + m_half_size.x) &&
           (point.y >= m_center.y - m_half_size.y &&
            point.y <= m_center.y + m_half_size.y) &&
           (point.z >= m_center.z - m_half_size.z &&
            point.z <= m_center.z + m_half_size.z);
  }
  ...
  bool intersects(const math::vec3 &center, const math::vec3 &half_size,
                  const math::vec3 &position, f32 radius) const {
    // Check for intersection between a cube and a sphere
    f32 dmin = 0;
    for (int i = 0; i < 3; i++) {
      if (position[i] < center[i] - half_size[i]) {
        dmin += (position[i] - (center[i] - half_size[i])) *
                (position[i] - (center[i] - half_size[i]));
  ...
  bool intersects(const math::vec3 &center, const math::vec3 &half_size,
                  const math::vec3 &a_min, const math::vec3 &a_max) const {
  ...
  std::vector<Entity> query(const math::vec3 &query_position,
                            f32 radius) const override {
    std::vector<Entity> result;
    if (intersects(m_center, m_half_size, query_position, radius)) {
      if (is_leaf()) {
        std::copy_if(
            m_entities.begin(), m_entities.end(), std::back_inserter(result),
            [&](const Entity &e) {
              math::vec3 entity_pos = extract_position(
                  Engine::get<ECS>().get_component<Transform>(e).m_transform);
        ...
        for (auto &child : m_children) {
          ...
          result.insert(result.end(), child_results.begin(),
                        child_results.end());
        }
      }
    }
  ...
  std::vector<Entity> query(const math::vec3 &min,
                            const math::vec3 &max) const override {
    std::vector<Entity> result;
...
private:
  void subdivide() {
    ...
    for (int i = 0; i < 8; ++i) {
      ...
      m_children[i] =
          std::make_unique<Octree>(child_center, child_half_size, this);
    }

    // Move entities to appropriate children
    for (const auto &e : m_entities) {
      auto position = extract_position(
          ecs.get_component<TransformComponent>(e).m_transform);
      i32 octant = get_octant(position);
      m_children[octant]->add_entity(e);
    }
  ...
  void merge() {
    ...
    for (auto &child : m_children) {
      ...
      m_entities.insert(m_entities.end(), child->m_entities.begin(),
  ...
  void collect_entities(std::vector<Entity> &all_entities) {
    if (is_leaf()) {
      all_entities.insert(all_entities.end(), m_entities.begin(),
                          m_entities.end());
    } else {
      for (auto &child : m_children) {
        if (child) {
