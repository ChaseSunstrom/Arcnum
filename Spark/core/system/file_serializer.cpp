...
namespace Spark
{
FileSerializer::FileSerializer(const std::filesystem::path &path) : m_path(path) { m_output = std::ofstream(path, std::ofstream::out | std::ofstream::binary); }
...
FileDeserializer::FileDeserializer(const std::filesystem::path &path) : m_path(path) { m_input = std::ifstream(path, std::ifstream::in | std::ifstream::binary); }
