...
namespace Spark
{
	...
	public:
		...
		template <typename V>
		void WriteMap(const std::unordered_map<std::string, V>& map)
		{
			WriteRaw(map.size());
			for (const auto& [key, value] : map)
			{
	...
	public:
		...
		template <typename V>
		void ReadMap(std::unordered_map<std::string, V>& map)
		{
			size_t size;
			ReadRaw(size);
			for (size_t i = 0; i < size; i++)
