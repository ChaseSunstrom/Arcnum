...
struct octree_fixture {
                ...
                math::vec3(50.0f)) // Large enough for multiple entities
  {
    ...
    for (i32 i = 0; i < 10; ++i) // Create 10 entities
    {
      ...
      Transform t;
      t.m_transform = math::translate(math::mat4(1.0f),
                                      math::vec3(distribution(generator),
...
TEST(test_octree_add_entity) {
  ...
  entities.push_back(
      create_transformed_entity(instance, math::vec3(0.0f, 0.0f, 0.0f)));
  entities.push_back(
      create_transformed_entity(instance, math::vec3(25.0f, 25.0f, 25.0f)));
  entities.push_back(
      create_transformed_entity(instance, math::vec3(-25.0f, -25.0f, -25.0f)));
  ...
  for (auto &ent : entities) {
    ...
    if (!is_inside) {
      ...
      std::cout << "Entity expected to be inside octree is outside. Position: "
...
TEST(test_octree_expansion) {
  ...
  Entity ent = create_transformed_entity(
      fixture._ecs, math::vec3(100.0f, 100.0f,
                               100.0f)); // Position outside the initial bounds
  fixture._octree.add_entity(ent);
  EXPECT_TRUE(fixture._octree.entity_is_inside(
      ent)); // Entity should now be inside after expansion
...
TEST(test_octree_update_entity) {
  ...
  t.m_transform =
      math::translate(math::mat4(1.0f), math::vec3(0.0f)); // Move to the origin
  fixture._ecs.set_component(ent, t);

  fixture._octree.update_entity(ent);
  EXPECT_TRUE(fixture._octree.entity_is_inside(
      ent)); // Check if the entity is inside after update
}
...
TEST(test_octree_query_with_filter) {
  ...
  for (auto &ent : results) {
    ...
    EXPECT_TRUE(extract_position(t.m_transform).x >
                0.0f); // Verify filter effectiveness
  }
}
...
TEST(test_octree_subdivision) {
  ...
  for (i32 i = 0; i < 20; ++i) {
    Entity ent = create_transformed_entity(
        fixture._ecs, math::vec3(0.0f)); // All entities at the origin
    fixture._octree.add_entity(ent);
  }

  EXPECT_FALSE(fixture._octree
                   .is_leaf()); // Octree should not be a leaf after subdivision
}
