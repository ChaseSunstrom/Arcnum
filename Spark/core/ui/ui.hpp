...
public:
  ...
  void update_text(const std::string &new_text) { m_text = new_text; }

  void draw() override { ImGui::Text("%s", m_text.c_str()); }
...
public:
  SliderFloatComponent(const std::string &id, const std::string &label,
                       f32 *value, f32 min, f32 max)
      : UIComponent(id), m_label(label), m_value(value), m_min(min),
        m_max(max) {}
...
public:
  ListBoxComponent(const std::string &id, const std::string &label,
                   i32 *current_item, std::vector<std::string> items)
      : UIComponent(id), m_label(label), m_current_item(current_item),
...
public:
  TreeComponent(const std::string &id, const std::string &label,
...
private:
  ...
  std::function<void()>
      m_content; // Function to draw the content of the tree node
};

class ColorPickerComponent : public UIComponent {
public:
  ColorPickerComponent(const std::string &id, const std::string &label,
                       f32 *color)
      : UIComponent(id), m_label(label), m_color(color) {}

  void draw() override { ImGui::ColorEdit3(m_label.c_str(), m_color); }
...
public:
  RadioButtonComponent(const std::string &id, const std::string &label,
                       i32 *selected, std::vector<std::string> options)
      : UIComponent(id), m_label(label), m_selected(selected),
...
public:
  WindowComponent(const std::string &id, const std::string &title)
      : UIComponent(id), m_title(title), m_p_open(nullptr),
        m_position(ImVec2(0.0f, 0.0f)), m_size(ImVec2(300.0f, 300.0f)) {}

  WindowComponent(const std::string &id, const std::string &title, bool *p_open,
                  const ImVec2 &position, const ImVec2 &size)
      : UIComponent(id), m_title(title), m_p_open(p_open), m_position(position),
        m_size(size) {}
  ...
  std::unordered_map<std::string, std::shared_ptr<UIComponent>> &
  get_children() {
    return m_children;
  }
...
public:
  ToggleSwitchComponent(const std::string &id, const std::string &label,
                        bool *value)
...
public:
  explicit MultiTextComponent(const std::string &id,
                              const std::vector<std::string> &multi_text)
      : UIComponent(id), m_text_lines(multi_text) {}

  void add_text(const std::string &text) { m_text_lines.push_back(text); }

  void clear_text() { m_text_lines.clear(); }
...
public:
  DropdownComponent(const std::string &id, const std::string &label,
                    i32 *current_item, std::vector<std::string> items)
      : UIComponent(id), m_label(label), m_current_item(current_item),
        m_items(std::move(items)) {
...
public:
  ProgressBarComponent(const std::string &id, const std::string &label,
                       f32 value, const ImVec2 &size = ImVec2(-1, 0))
...
public:
  TextInputComponent(const std::string &id, const std::string &label,
                     char *buffer, size_t buffer_size)
      : UIComponent(id), m_label(label), m_buffer(buffer),
...
public:
  CheckBoxComponent(const std::string &id, const std::string &label,
                    bool *value)
      : UIComponent(id), m_label(label), m_value(value) {}

  void draw() override { ImGui::Checkbox(m_label.c_str(), m_value); }
...
public:
  SliderIntComponent(const std::string &id, const std::string &label,
                     i32 *value, i32 min, i32 max)
      : UIComponent(id), m_label(label), m_value(value), m_min(min),
        m_max(max) {}
...
struct UITheme {
  struct ColorScheme {
    ...
    ColorScheme()
        : m_text(ImGui::GetStyle().Colors[ImGuiCol_Text]),
          ...
          m_nav_windowing_highlight(
              ImGui::GetStyle().Colors[ImGuiCol_NavWindowingHighlight]),
          m_nav_windowing_dim_bg(
              ImGui::GetStyle().Colors[ImGuiCol_NavWindowingDimBg]),
          m_modal_window_dim_bg(
  ...
  void apply() const {
    ...
    style.Colors[ImGuiCol_ScrollbarGrabHovered] =
        m_colors.m_scrollbar_grab_hovered;
    style.Colors[ImGuiCol_ScrollbarGrabActive] =
        m_colors.m_scrollbar_grab_active;
    ...
    style.Colors[ImGuiCol_PlotHistogramHovered] =
        m_colors.m_plot_histogram_hovered;
    ...
    style.Colors[ImGuiCol_NavWindowingHighlight] =
        m_colors.m_nav_windowing_highlight;
...
public:
  ...
  void create(const std::string &window_name, const std::string &id,
              Args &&...args) {
    ...
    if (!window_name.empty() &&
        m_windows.find(window_name) != m_windows.end()) {
  ...
  void set_theme(const UITheme &theme) { theme.apply(); }
...
private:
  ...
  std::unordered_map<std::string, std::unique_ptr<WindowComponent>>
      m_windows; // Windows and their components
};
