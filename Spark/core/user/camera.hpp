...
struct Camera {
  Camera(const math::vec3 &position = math::vec3(0.0f, 0.0f, 3.0f),
         ...
         f32 fov = 90.0f, f32 aspect_ratio = 1, f32 near_plane = 0.1f,
         f32 far_plane = 100.0f)
      : m_position(position), m_direction(direction), m_fov(fov),
        m_aspect_ratio(aspect_ratio), m_near_plane(near_plane),
        m_far_plane(far_plane) {
    m_frustum = std::make_unique<Frustum>(get_view_projection_matrix());
  }

  math::mat4 get_view_matrix() const {
    return math::lookAt(m_position, m_position + m_direction,
                        math::vec3(0.0f, 1.0f, 0.0f));
  }

  math::mat4 get_projection_matrix() const {
    return math::perspective(math::radians(m_fov), m_aspect_ratio, m_near_plane,
                             m_far_plane);
  }
  ...
  bool operator!=(const Camera &other) const {
    return !(
        ...
        m_fov == other.m_fov && m_aspect_ratio == other.m_aspect_ratio &&
