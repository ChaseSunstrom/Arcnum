	...
	template<typename CharType = char, typename Allocator = _SPARK Allocator<CharType>> class BasicString {
	  public:
		...
		BasicString(const CharType* data)
			: m_size(StringLength(data)), m_capacity(m_size + 1) {
		...
		template<typename T>
		BasicString operator+(T value) const {
			BasicString result(*this);
			result += value;
			return result;
		}
		...
		bool operator==(const BasicString& other) const {
			return (m_size == other.m_size) && (StringCompare(m_data, other.m_data) == 0);
		}
		...
		bool operator<(const BasicString& other) const {
			return StringCompare(m_data, other.m_data) < 0;
		}
	  ...
	  private:
		...
		static i32 StringCompare(const CharType* s1, const CharType* s2, SizeType n = npos) {
			if (n == npos) {
				...
				return n == npos ? 0 : *(const unsigned char*) s1 - *(const unsigned char*) s2;
			}
		}
		...
		static void StringCopyN(CharType* dest, const CharType* src, SizeType n) {
			while (n--) {
				*dest++ = *src++;
			}
		}
		...
		void ReallocateAndCopy(SizeType new_capacity) {
			...
			if (m_data) {
				StringCopy(new_data, m_data, m_size + 1);
				AllocatorTraits::Deallocate(m_allocator, m_data, m_capacity);
			} else {
				new_data[0] = '\0';
			}
		...
		void Append(const CharType* str, SizeType len) {
			...
			StringCopyN(m_data + m_size, str, len);
			m_size = new_size;
			m_data[m_size] = '\0';
		}
		...
		template<typename T>
		static BasicString ToString(T value) {
