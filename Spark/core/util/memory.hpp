...
namespace std
{
template <typename TO, typename FROM> unique_ptr<TO> static_pointer_cast(unique_ptr<FROM> &&old)
{
...
{
  public:
    explicit UniqueShieldPtr(T *ptr = nullptr) : m_data(ptr), m_mutex(new std::recursive_mutex)
    {
    }
...
template <typename T> class SharedProxy
{
  ...
  private:
    ...
    SharedProxy(std::shared_ptr<T> ptr, std::recursive_mutex &mtx) : m_ptr(std::move(ptr)), m_lock(mtx)
    {
    }
...
template <typename T> class SharedShieldPtr
{
  public:
    ...
    std::shared_ptr<std::recursive_mutex> get_mutex() const
    {
        return m_mutex;
    }
...
template <typename T, typename... Args> UniqueShieldPtr<T> make_unique_shield(Args &&...args)
{
    return UniqueShieldPtr<T>(new T(std::forward<Args>(args)...));
}

template <typename T, typename... Args> SharedShieldPtr<T> make_shared_shield(Args &&...args)
{
    return SharedShieldPtr<T>(std::make_shared<T>(std::forward<Args>(args)...));
}

template <typename T> UniqueShieldPtr<T> make_threaded(std::unique_ptr<T> &&ptr)
{
...
template <typename T, typename U> UniqueShieldPtr<T> threaded_pointer_cast(const UniqueShieldPtr<U> &ptr)
{
    return UniqueShieldPtr<T>((T *)ptr.get(), ptr.get_mutex());
}

template <typename T, typename U> SharedShieldPtr<T> threaded_pointer_cast(const SharedShieldPtr<U> &ptr)
{
