...
namespace Spark {
	...
	template <typename _Ty, typename... Args> SharedPtr<_Ty> MakeShared(Args&&... args) {
		return SharedPtr<_Ty>(new _Ty(std::forward<Args>(args)...));
	}

	template <typename _Ty> SharedPtr<_Ty> MakeShared(const _Ty* ptr) {
		return SharedPtr<_Ty>(ptr);
	}

	template <typename _Ty> SharedPtr<_Ty> MakeShared(const _Ty& ref) {
		return SharedPtr<_Ty>(&ref);
	}
