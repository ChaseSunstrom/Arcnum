...
namespace Spark
{
ThreadPool::ThreadPool(u32 num_threads) : m_stop(false), m_active_tasks(0), m_sync_tasks(0) { Initialize(num_threads); }
...
bool ThreadPool::SyncRegisteredTasks(std::chrono::milliseconds timeout)
{
	...
	bool						 synced = m_sync_condition.wait_for(lock,
											timeout,
											[this, &start]
											{
