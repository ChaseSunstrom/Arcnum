...
{
    bool operator()(const std::pair<TaskPriority, std::function<void()>> &lhs,
                        ...
                        Args &&...args) -> std::future<typename std::invoke_result<F, Args...>::type>
    {
        ...
        std::future<return_type> res = task->get_future();
        {
            ...
            auto min_it = std::min_element(
                s_tasks_queues.begin(), s_tasks_queues.end(),
                [](const TaskQueue &a, const TaskQueue &b) { return a.m_queue.size() < b.m_queue.size(); });
  ...
  private:
    static void worker_thread(std::shared_ptr<ThreadControlBlock> tcb, u32 index)
    {
        ...
        while (true)
        {
            ...
            {
                std::unique_lock<std::mutex> lock(s_queue_mutex);
                s_condition.wait(lock, [index] { return s_stop || !s_tasks_queues[index].m_queue.empty(); });
  ...
  private:
    struct TaskQueue
    {
        ...
        std::priority_queue<std::pair<TaskPriority, std::function<void()>>,
                            std::vector<std::pair<TaskPriority, std::function<void()>>>, TaskComparator>
            m_queue;
    ...
    static inline std::atomic<bool> s_stop{0};
};
} // namespace Spark
