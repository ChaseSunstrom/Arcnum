...
struct TaskComparator {
  bool
  operator()(const std::pair<TaskPriority, std::function<void()>> &lhs,
...
public:
  ...
  static auto enqueue(TaskPriority priority, bool synchronize, F &&f,
                      Args &&...args)
    ...
    std::future<return_type> res = task->get_future();
    {
      ...
      // Find the least loaded queue or with the highest priority task to insert
      // this task
      auto min_it =
          std::min_element(s_tasks_queues.begin(), s_tasks_queues.end(),
                           [](const TaskQueue &a, const TaskQueue &b) {
      ...
      min_it->m_queue.emplace(priority, [task, synchronize]() {
        ...
        if (synchronize) {
          auto it = std::find_if(
              s_threads_control.begin(), s_threads_control.end(),
  ...
  static void sync_this_thread(bool register_for_sync) {
    auto it =
        std::find_if(s_threads_control.begin(), s_threads_control.end(),
                     [](const std::shared_ptr<ThreadControlBlock> &tcb) {
                       return tcb->thread_id == std::this_thread::get_id();
                     });
  ...
  static void synchronize_registered_threads() {
    ...
    s_sync_condition.wait(lock, [] {
      return std::all_of(s_threads_control.begin(), s_threads_control.end(),
                         [](const std::shared_ptr<ThreadControlBlock> &tcb) {
                           return !tcb->is_registered_for_sync ||
                                  tcb->has_reached_sync_point;
                         });
    });
...
private:
  static void worker_thread(std::shared_ptr<ThreadControlBlock> tcb,
                            u32 index) {
    ...
    while (true) {
      std::function<void()> task;
      {
        ...
        if (s_stop && s_tasks_queues[index].m_queue.empty()) {
          return; // Exit the thread if the pool is stopped and no tasks are
                  // left
        }
...
private:
  struct TaskQueue {
    ...
    std::priority_queue<
        std::pair<TaskPriority, std::function<void()>>,
  ...
  static inline std::vector<std::shared_ptr<ThreadControlBlock>>
      s_threads_control;
