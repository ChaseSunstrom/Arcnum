...
struct VulkanMesh : public Mesh
{
  public:
    ...
    VulkanMesh(const std::vector<Vertex> &vertices, const std::vector<u32> &indices, const UBOTypes &...ubo_data)
    ...
    template <typename... UBOTypes>
    void update(const std::vector<Vertex> &vertices, const std::vector<u32> &indices, const UBOTypes &...ubo_data)
    {
        m_vertices = vertices;
        m_indices = indices;
        (add_ubo(ubo_data), ...);
  ...
  private:
    ...
    void copy_buffer(internal::VkBuffer src_buffer, internal::VkBuffer dst_buffer, internal::VkDeviceSize size)
    {
        ...
        vkFreeCommandBuffers(vk_window.get_window_data().m_device, vk_window.get_window_data().m_command_pool, 1,
                             &command_buffer);
    }
    ...
    template <typename... UBOTypes> void update_descriptor_sets(const UBOTypes &...ubo_data)
    {
        ...
        for (u64 i = 0; i < vk_window.get_window_data().m_max_frames_in_flight; i++)
        {
            ...
            // index.
            update_descriptor_set_impl(device, descriptor_set, std::index_sequence_for<UBOTypes...>{}, buffer_infos,
                                       write_descriptor_sets, ubo_data...);
        }
    }
