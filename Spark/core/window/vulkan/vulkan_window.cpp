...
VulkanWindow::VulkanWindow() : Window(WindowType::VULKAN) {
  m_window_data = std::make_unique<VulkanWindowData>("Title", false, 1080, 1080,
                                                     event_callback);
...
VulkanWindow::~VulkanWindow() {
  ...
  vkDestroyDescriptorSetLayout(m_window_data->m_device,
                               m_window_data->m_descriptor_set_layout, nullptr);
  ...
  vkDestroyPipeline(m_window_data->m_device, m_window_data->m_graphics_pipeline,
                    nullptr);
  vkDestroyPipelineLayout(m_window_data->m_device,
                          m_window_data->m_pipeline_layout, nullptr);
  vkDestroyRenderPass(m_window_data->m_device, m_window_data->m_render_pass,
                      nullptr);

  for (u64 i = 0; i < m_window_data->m_max_frames_in_flight; i++) {
    vkDestroySemaphore(m_window_data->m_device,
                       m_window_data->m_image_available_semaphores[i], nullptr);
    vkDestroySemaphore(m_window_data->m_device,
                       m_window_data->m_render_finished_semaphores[i], nullptr);
    vkDestroyFence(m_window_data->m_device,
                   m_window_data->m_in_flight_fences[i], nullptr);
  }

  vkDestroyCommandPool(m_window_data->m_device, m_window_data->m_command_pool,
                       nullptr);
  ...
  vkDestroySwapchainKHR(m_window_data->m_device, m_window_data->m_swapchain,
                        nullptr);

  if (m_enable_validation_layers) {
    destroy_debug_utils_messenger_ext(
        m_window_data->m_instance, m_window_data->m_debug_messenger, nullptr);
  }

  vkDestroySurfaceKHR(m_window_data->m_instance, m_window_data->m_surface,
                      nullptr);
...
void VulkanWindow::draw_frame() {
  vkWaitForFences(
      m_window_data->m_device, 1,
      &m_window_data->m_in_flight_fences[m_window_data->m_current_frame],
      VK_TRUE, UINT64_MAX);

  u32 image_index;
  VkResult result = vkAcquireNextImageKHR(
      m_window_data->m_device, m_window_data->m_swapchain, UINT64_MAX,
      m_window_data
          ->m_image_available_semaphores[m_window_data->m_current_frame],
  ...
  vkResetCommandBuffer(
      m_window_data->m_command_buffers[m_window_data->m_current_frame], 0);

  record_command_buffer(
      m_window_data->m_command_buffers[m_window_data->m_current_frame],
      image_index);
  ...
  VkSemaphore signal_semaphores[] = {
      m_window_data
          ->m_render_finished_semaphores[m_window_data->m_current_frame]};
  submit_info.signalSemaphoreCount = 1;
  submit_info.pSignalSemaphores = signal_semaphores;

  if (vkQueueSubmit(
          m_window_data->m_graphics_queue, 1, &submit_info,
          m_window_data->m_in_flight_fences[m_window_data->m_current_frame]) !=
      VK_SUCCESS) {
  ...
  if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR ||
      m_window_data->m_framebuffer_resized) {
  ...
  m_window_data->m_current_frame = (m_window_data->m_current_frame + 1) %
                                   m_window_data->m_max_frames_in_flight;
}

void VulkanWindow::pre_draw() { glfwPollEvents(); }

void VulkanWindow::on_update() { draw_frame(); }

void VulkanWindow::post_draw() {}

bool VulkanWindow::is_running() { return !glfwWindowShouldClose(m_window); }
...
void VulkanWindow::init_physical_device() {
  ...
  vkEnumeratePhysicalDevices(m_window_data->m_instance, &device_count,
                             devices.data());
...
VulkanWindow::find_queue_families(VkPhysicalDevice device) {
  ...
  for (const auto &queue_family : queue_families) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(device, i, m_window_data->m_surface,
                                         &present_support);
...
bool VulkanWindow::check_device_extension_support(VkPhysicalDevice device) {
  ...
  std::set<std::string> required_extensions(
      m_window_data->m_device_extensions.begin(),
...
i32 VulkanWindow::rate_device(VkPhysicalDevice device) {
  ...
  score += static_cast<i32>(total_memory >>
                            20); // Add one point per MB of GPU memory
...
void VulkanWindow::cleanup_swap_chain() {
  for (u64 i = 0; i < m_window_data->m_swapchain_framebuffers.size(); i++) {
    vkDestroyFramebuffer(m_window_data->m_device,
                         m_window_data->m_swapchain_framebuffers[i], nullptr);
  }

  for (u64 i = 0; i < m_window_data->m_swapchain_image_views.size(); i++) {
    vkDestroyImageView(m_window_data->m_device,
                       m_window_data->m_swapchain_image_views[i], nullptr);
  }
...
void VulkanWindow::populate_debug_messenger(
...
void VulkanWindow::init_sync_objects() {
  m_window_data->m_image_available_semaphores.resize(
      m_window_data->m_max_frames_in_flight);
  m_window_data->m_render_finished_semaphores.resize(
      m_window_data->m_max_frames_in_flight);
  m_window_data->m_in_flight_fences.resize(
      m_window_data->m_max_frames_in_flight);
  ...
  for (u64 i = 0; i < m_window_data->m_max_frames_in_flight; i++) {
    if (vkCreateSemaphore(m_window_data->m_device, &semaphore_info, nullptr,
                          &m_window_data->m_image_available_semaphores[i]) !=
            VK_SUCCESS ||
        vkCreateSemaphore(m_window_data->m_device, &semaphore_info, nullptr,
                          &m_window_data->m_render_finished_semaphores[i]) !=
            VK_SUCCESS ||
        vkCreateFence(m_window_data->m_device, &fence_info, nullptr,
                      &m_window_data->m_in_flight_fences[i]) != VK_SUCCESS) {
...
void VulkanWindow::init_gl() {
  ...
  m_window = glfwCreateWindow(m_window_data->m_width, m_window_data->m_height,
                              m_window_data->m_title.c_str(), NULL, NULL);
...
void VulkanWindow::init_vulkan() {
  ...
  if (m_enable_validation_layers) {
    create_info.enabledLayerCount =
        static_cast<u32>(m_window_data->m_validation_layers.size());
  ...
  VkResult result =
      vkCreateInstance(&create_info, nullptr, &m_window_data->m_instance);
}
...
VulkanWindow::query_swap_chain_support(VkPhysicalDevice device) {
  ...
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, m_window_data->m_surface,
                                            &details.m_capabilities);

  u32 format_count;
  vkGetPhysicalDeviceSurfaceFormatsKHR(device, m_window_data->m_surface,
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(device, m_window_data->m_surface,
...
void VulkanWindow::init_swap_chain() {
  VulkanSwapChainSupportDetails swap_chain_support =
      query_swap_chain_support(m_window_data->m_physical_device);

  VkSurfaceFormatKHR surface_format =
      choose_swap_surface_format(swap_chain_support.m_formats);
  VkPresentModeKHR present_mode =
      choose_swap_present_mode(swap_chain_support.m_present_modes);
  ...
  VulkanQueueFamilyIndices indices =
      find_queue_families(m_window_data->m_physical_device);
  u32 queue_family_indices[] = {indices.m_graphics_family.value(),
  ...
  VkResult result = vkCreateSwapchainKHR(m_window_data->m_device, &create_info,
                                         nullptr, &m_window_data->m_swapchain);
  ...
  vkGetSwapchainImagesKHR(m_window_data->m_device, m_window_data->m_swapchain,
                          &image_count, nullptr);
...
void VulkanWindow::init_logical_device() {
  ...
  std::set<u32> unique_queue_families = {indices.m_graphics_family.value(),
                                         indices.m_present_family.value()};
  ...
  create_info.enabledExtensionCount =
      static_cast<u32>(m_window_data->m_device_extensions.size());
  create_info.ppEnabledExtensionNames =
      m_window_data->m_device_extensions.data();
  ...
  if (vkCreateDevice(m_window_data->m_physical_device, &create_info, nullptr,
                     &m_window_data->m_device) != VK_SUCCESS) {
    SPARK_ERROR("[VULKAN] Failed to create logical device!");
    assert(false);
  }

  vkGetDeviceQueue(m_window_data->m_device, indices.m_graphics_family.value(),
                   0, &m_window_data->m_graphics_queue);
  vkGetDeviceQueue(m_window_data->m_device, indices.m_present_family.value(), 0,
                   &m_window_data->m_present_queue);
}

void VulkanWindow::init_pipeline() {
  std::vector<char> vertex_shader_code =
      read_file_to_bytes("Spark/shaders/vertex_shader.spv");
  std::vector<char> fragment_shader_code =
      read_file_to_bytes("Spark/shaders/fragment_shader.spv");
  ...
  if (vkCreatePipelineLayout(m_window_data->m_device, &pipeline_layout_info,
                             nullptr,
  ...
  if (vkCreateGraphicsPipelines(
          m_window_data->m_device, VK_NULL_HANDLE, 1, &pipeline_info, nullptr,
          &m_window_data->m_graphics_pipeline) != VK_SUCCESS) {
    SPARK_ERROR("[VULKAN] Failed to create graphics pipeline!");
    assert(false);
  }

  vkDestroyShaderModule(m_window_data->m_device, fragment_shader_module,
                        nullptr);
...
VulkanWindow::create_shader_module(const std::vector<char> &code) {
  ...
  if (vkCreateShaderModule(m_window_data->m_device, &create_info, nullptr,
...
VulkanWindow::choose_swap_extent(const VkSurfaceCapabilitiesKHR &capabilities) {
  if (capabilities.currentExtent.width != std::numeric_limits<u32>::max()) {
    ...
    VkExtent2D actual_extent = {static_cast<u32>(width),
...
void VulkanWindow::init_surface() {
  if (glfwCreateWindowSurface(m_window_data->m_instance, m_window, nullptr,
                              &m_window_data->m_surface) != VK_SUCCESS) {
...
void VulkanWindow::init_image_views() {
  m_window_data->m_swapchain_image_views.resize(
      m_window_data->m_swapchain_images.size());
...
void VulkanWindow::init_render_pass() {
  ...
  if (vkCreateRenderPass(m_window_data->m_device, &render_pass_info, nullptr,
                         &m_window_data->m_render_pass) != VK_SUCCESS) {
...
void VulkanWindow::init_framebuffers() {
  m_window_data->m_swapchain_framebuffers.resize(
      m_window_data->m_swapchain_image_views.size());
  ...
  for (u64 i = 0; i < m_window_data->m_swapchain_image_views.size(); i++) {
    ...
    if (vkCreateFramebuffer(m_window_data->m_device, &framebuffer_info, nullptr,
                            &m_window_data->m_swapchain_framebuffers[i]) !=
        VK_SUCCESS) {
...
void VulkanWindow::init_command_pool() {
  ...
  if (vkCreateCommandPool(m_window_data->m_device, &pool_info, nullptr,
                          &m_window_data->m_command_pool) != VK_SUCCESS) {
    SPARK_ERROR("[VULKAN] Failed to create command pool!");
    assert(false);
  }
}

void VulkanWindow::record_command_buffer(VkCommandBuffer command_buffer,
                                         u32 image_index) {
  ...
  render_pass_info.framebuffer =
      m_window_data->m_swapchain_framebuffers[image_index];
  render_pass_info.renderArea.offset = {0, 0};
  render_pass_info.renderArea.extent = m_window_data->m_swapchain_extent;

  Spark::math::vec4 scene_color = Engine::get<SceneManager>()
                                      .get_current_scene()
                                      .get_scene_config()
                                      .m_background_color;

  VkClearValue clear_color = {
      {{scene_color.r, scene_color.g, scene_color.b, scene_color.a}}};
  ...
  vkCmdBindPipeline(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                    m_window_data->m_graphics_pipeline);
...
void VulkanWindow::init_command_buffers() {
  m_window_data->m_command_buffers.resize(
      m_window_data->m_max_frames_in_flight);
  ...
  alloc_info.commandBufferCount =
      static_cast<u32>(m_window_data->m_command_buffers.size());

  if (vkAllocateCommandBuffers(
          m_window_data->m_device, &alloc_info,
          &m_window_data->m_command_buffers[m_window_data->m_current_frame]) !=
      VK_SUCCESS) {
...
void VulkanWindow::init_descriptor_pool() {
  ...
  pool_size.descriptorCount =
      static_cast<u32>(m_window_data->m_max_frames_in_flight);
  ...
  if (vkCreateDescriptorPool(m_window_data->m_device, &pool_info, nullptr,
                             &m_window_data->m_descriptor_pool) != VK_SUCCESS) {
...
void VulkanWindow::init_descriptor_sets() {
  ...
  m_window_data->m_descriptor_sets.resize(
      m_window_data->m_max_frames_in_flight);

  if (vkAllocateDescriptorSets(m_window_data->m_device, &alloc_info,
                               m_window_data->m_descriptor_sets.data()) !=
      VK_SUCCESS) {
...
void VulkanWindow::init_debug() {
  ...
  if (create_debug_utils_messenger_ext(
          m_window_data->m_instance, &create_info, nullptr,
...
void VulkanWindow::init_descriptor_set() {
  ...
  if (vkCreateDescriptorSetLayout(
          m_window_data->m_device, &layout_info, nullptr,
...
void VulkanWindow::framebuffer_resize_callback(GLFWwindow *window, i32 width,
                                               i32 height) {
  auto window_data =
      reinterpret_cast<VulkanWindowData *>(glfwGetWindowUserPointer(window));
  window_data->m_framebuffer_resized = true;
}

void VulkanWindow::close_event_callback(GLFWwindow *window) {
  VulkanWindowData *_window_data =
      static_cast<VulkanWindowData *>(glfwGetWindowUserPointer(window));
  auto event = std::make_shared<WindowClosedEvent>();

  _window_data->m_event_callback(event);
}

void VulkanWindow::key_event_callback(GLFWwindow *window, i32 key, i32 scancode,
                                      i32 action, i32 mods) {
  VulkanWindowData *_window_data =
      static_cast<VulkanWindowData *>(glfwGetWindowUserPointer(window));
...
void VulkanWindow::mouse_button_event_callback(GLFWwindow *window, i32 button,
                                               i32 action, i32 mods) {
  VulkanWindowData *_window_data =
      static_cast<VulkanWindowData *>(glfwGetWindowUserPointer(window));
...
void VulkanWindow::mouse_scroll_event_callback(GLFWwindow *window, f64 xoffset,
                                               f64 yoffset) {
  VulkanWindowData *_window_data =
      static_cast<VulkanWindowData *>(glfwGetWindowUserPointer(window));
...
void VulkanWindow::mouse_move_event_callback(GLFWwindow *window, f64 xpos,
                                             f64 ypos) {
  VulkanWindowData *_window_data =
      static_cast<VulkanWindowData *>(glfwGetWindowUserPointer(window));
