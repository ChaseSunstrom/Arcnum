...
void on_start() {
  ...
  spark::AssetManager::create_asset<spark::Audio>("retro",
                                                  "assets/sfx/retro.wav");

  cur_scene.set_background_color(spark::math::vec4(1.0f, 0.0f, 0.0f, 1.0f));

  // Create material and mesh
  spark::Material &material =
      spark::AssetManager::create_asset<spark::Material>(
          "material", spark::math::vec4(1.0f, 1.0f, 1.0f, 1.0f));
  std::default_random_engine random_engine;
  std::uniform_real_distribution<spark::f32> distribution(0.0f, 1.0f);

  std::vector<spark::Vertex> vertices = {{{-0.5f, -0.5f, 0.0f}},
                                         {{0.5f, -0.5f, 0.0f}},
                                         {{0.5f, 0.5f, 0.0f}},
                                         {{-0.5f, 0.5f, 0.0f}}};
  ...
  _ubo.m_view = spark::math::lookAt(spark::math::vec3(0.0f, 0.0f, 3.0f),
                                    spark::math::vec3(0.0f, 0.0f, 0.0f),
                                    spark::math::vec3(0.0f, 1.0f, 0.0f));

  spark::AssetManager::create_asset<spark::Mesh>("quad", vertices, indices,
                                                 _ubo);

  spark::Entity e = ecs.create_entity(spark::MeshComponent("quad"),
...
bool on_event(std::shared_ptr<spark::Event> event) {
  ...
  case KEY_PRESSED_EVENT: {
    auto received_event =
        std::dynamic_pointer_cast<spark::KeyPressedEvent>(event);

    if (received_event) {
      SPARK_TRACE("[KEY PRESSED]: "
                  << spark::to_string(spark::Key(received_event->m_key_code)));
    }
  } break;
  case KEY_REPEAT_EVENT: {
    auto received_event =
        std::dynamic_pointer_cast<spark::KeyRepeatedEvent>(event);

    if (received_event) {
      SPARK_TRACE("[KEY HELD]: "
                  << spark::to_string(spark::Key(received_event->m_key_code)));
    }
  } break;
  case KEY_RELEASED_EVENT: {
    auto received_event =
        std::dynamic_pointer_cast<spark::KeyReleasedEvent>(event);

    if (received_event) {
      SPARK_TRACE("[KEY RELEASED]: "
                  << spark::to_string(spark::Key(received_event->m_key_code)));
    }
  } break;
  case UDP_SERVER_RECEIVE_EVENT: {
    ...
    auto received_event =
        std::dynamic_pointer_cast<spark::net::UDPServerReceiveEvent>(event);
    if (received_event) {
      std::string message =
          dynamic_cast<spark::net::ChatMessage *>(&*received_event->m_packet)
              ->m_message; // Convert packet to string

      // Find the text component meant to display messages
      auto text_comp =
          _ui_manager.find_component_by_id<spark::MultiTextComponent>("Text");
      if (text_comp) {
        text_comp->add_text(
            message); // Update the text component with the new message
      }
    }
  } break;
  }
